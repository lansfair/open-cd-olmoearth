...
class OlmoEarthBackbone(BaseModule):
    """Dense OLMoEarth encoder backbone for Open-CD.

    The input tensor is flattened band-major as ``(B, C*T, H, W)``. Metadata
    such as timestamps and present bands is supplied by the change detector
    through :meth:`set_batch_metainfo`.
    """
    ...
    def init_weights(self) -> None:
        ...
        if checkpoint_path is None:
            raise ValueError("OlmoEarthBackbone init_cfg requires a checkpoint path.")
    ...
    def set_batch_metainfo(self, batch_metainfo: list[dict[str, Any]] | None) -> None:
    ...
    def _timestamps_from_metainfo(self, batch_size: int, device: torch.device) -> Tensor:
    ...
    def _make_sample(self, inputs: Tensor):
        ...
        image = inputs.reshape(batch_size, num_bands, self.num_timesteps, height, width)
        ...
        kwargs = {
            ...
            "timestamps": self._timestamps_from_metainfo(batch_size, inputs.device),
        }
