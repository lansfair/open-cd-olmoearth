...
def _append_suffix_if_needed(filename: str, suffix: str) -> str:
...
class OLMoEarthLEVIRCDDataset(_BaseCDDataset):
    ...
    The image pair is loaded by ``LoadOLMoEarthLEVIRPair`` as two RGB images and
    converted to OLMoEarth Sentinel-2 L2A input. Label PNGs are expected to use
    0 for unchanged/background and 255 for changed/foreground.
    """
    ...
    def _build_data_info(self, rel_img_path: str) -> dict:
        img_dir_from = self.data_prefix.get("img_path_from")
        img_dir_to = self.data_prefix.get("img_path_to")
        ...
        stem = (
            rel_img_path[: -len(self.img_suffix)]
            if rel_img_path.endswith(self.img_suffix)
            else rel_img_path
        )

        data_info = dict(
            img_path=[
                osp.join(img_dir_from, rel_img_path),
                osp.join(img_dir_to, rel_img_path),
            ],
    ...
    def load_data_list(self) -> List[dict]:
        """Load pairs from ann_file or scan the A/B folders.

        ``ann_file`` may contain either stems such as ``test_1`` or filenames
        such as ``test_1.png``. If ``ann_file`` is omitted, this scans
        ``img_path_from`` recursively and requires matching relative filenames
        in ``img_path_to``.
        """
        img_dir_from = self.data_prefix.get("img_path_from")
        img_dir_to = self.data_prefix.get("img_path_to")
        ...
        if ann_file and not osp.isabs(ann_file):
        ...
        if ann_file and osp.isfile(ann_file):
            for line in mmengine.list_from_file(ann_file, backend_args=self.backend_args):
                ...
                if not item:
                    continue
                rel_img_path = _append_suffix_if_needed(item, self.img_suffix)
                data_list.append(self._build_data_info(rel_img_path))
            return data_list
        ...
        if from_files != to_files:
            ...
            raise AssertionError(
                "A/B image filenames are not one-to-one correspondence. "
                f"Missing in B examples: {missing_in_b}; "
                f"missing in A examples: {missing_in_a}"
            )

        for rel_img_path in from_files:
            data_list.append(self._build_data_info(rel_img_path))
        return data_list
...
class LoadOLMoEarthLEVIRPair(BaseTransform):
    """Load two RGB dates and convert both to OLMoEarth Sentinel-2 L2A.

    Both timestamps are RGB PNGs. The transform maps them to the Sentinel-2 L2A
    modality expected by OLMoEarth:

      R -> B04, G -> B03, B -> B02

    The remaining Sentinel-2 bands are zero-filled in the tensor and excluded
    from OLMoEarth computation through ``present_bands`` metadata.
    """
    ...
    def __init__(
        ...
        expected_shape: tuple[int, int] | None = (128, 128),
        ...
        self.expected_shape = (
            tuple(int(x) for x in expected_shape)
        ...
        self.modality = "sentinel2_l2a"
        self.band_names = list(get_modality_bands(self.modality))
        self.norm_config = _load_computed_norm(self.modality)
        self.present_bands = list(RGB_TO_SENTINEL2_L2A.values())
        self.missing_bands = [
            band for band in SENTINEL2_L2A_BANDS if band not in self.present_bands
        ]
    ...
    def _check_shape(self, pre_rgb: np.ndarray, post_rgb: np.ndarray) -> None:
        if pre_rgb.shape[:2] != post_rgb.shape[:2]:
            raise ValueError(
                "LEVIR-CD A/B image shapes do not match: "
                f"{pre_rgb.shape} vs {post_rgb.shape}"
            )
        if self.expected_shape is not None and pre_rgb.shape[:2] != self.expected_shape:
            raise ValueError(
                "Unexpected LEVIR-CD image size. "
                f"Expected {self.expected_shape}, got {pre_rgb.shape[:2]}. "
                "Set expected_shape=None in the config if you want to disable "
                "this check."
            )

    def _rgb_to_s2(self, image: np.ndarray) -> np.ndarray:
        return rgb_to_pseudo_s2(
            ...
            clip_to_s2_scale=self.clip_to_s2_scale,
            std_multiplier=self.std_multiplier,
        )

    def _make_metainfo(self, timestamp: tuple[int, int, int], source: str) -> dict:
        return dict(
            olmoearth_modality=self.modality,
            olmoearth_num_timesteps=1,
            olmoearth_band_names=list(self.band_names),
            # Only B04/B03/B02 are valid observations. The backbone turns all
            # other Sentinel-2 bands into MISSING tokens from this metadata.
            present_bands=list(self.present_bands),
            missing_bands=list(self.missing_bands),
            timestamps=np.asarray([timestamp], dtype=np.int64),
            source=source,
            rgb_to_s2_mapping=dict(RGB_TO_SENTINEL2_L2A),
            input_value_range=self.input_value_range,
            input_max_value=self.input_max_value,
            target_s2_scale=self.target_s2_scale,
        )
    ...
    def transform(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        self._check_shape(pre_rgb, post_rgb)

        results["img"] = [
            self._rgb_to_s2(pre_rgb),
            self._rgb_to_s2(post_rgb),
        ]
        results["img_shape"] = pre_rgb.shape[:2]
        results["ori_shape"] = pre_rgb.shape[:2]
        results["olmoearth_from_metainfo"] = self._make_metainfo(
            self.pre_timestamp,
            "levir_cd_A_rgb_to_sentinel2_l2a",
        )
        results["olmoearth_to_metainfo"] = self._make_metainfo(
            self.post_timestamp,
            "levir_cd_B_rgb_to_sentinel2_l2a",
        )
        return results
...
class LoadOLMoEarthLEVIRAnnotations(BaseTransform):
    """Load LEVIR-CD binary labels.

    Raw labels:
      0   -> unchanged/background -> class id 0
      255 -> changed/foreground   -> class id 1

    ``255`` is therefore not treated as ignore here. Padding/ignore pixels
    introduced later by the data preprocessor still use ignore_index=255.
    """
    ...
    def transform(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        # Convert 0/255 labels to class ids 0/1.
