...
# Change this path to your local/server OLMoEarth-v1-Base directory.
...
model = dict(
    ...
    pretrained=None,
    # Each date is a 12-band Sentinel-2 L2A tensor. Two dates are concatenated
    # by PackOLMoEarthCDInputs, so the actual model input has 24 channels.
    ...
    backbone=dict(
        ...
        # Important: missing Sentinel-2 bands are masked, so do not use the
        # OLMoEarth fast path that assumes all bands are valid.
        fast_pass=False,
