...
class Masker(object):
    ...
    def _get_modalities(self, batch_keys):
        ...
        modalities = [
            mod
            for mod in batch_keys
            if not (
                mod.endswith("_dates")
                or mod.endswith("_mask")
                or mod.endswith("_density")
                or mod.endswith("cloud_density")
            )
        ]
        modalities = [
            mod
    ...
    def __call__(self, batch):
        ...
        raise NotImplementedError(
            "Masker is an abstract class. Please implement the __call__ method."
        )
...
class RandomMasker(Masker):
    ...
    def _mask_Spatial(self, B, patched_input_size, device="cpu"):
        ...
        if self.crop_percentage_axes < 1.0:
            ...
            h_crop, w_crop = (
                math.floor(patched_input_size[0] * self.crop_percentage_axes),
                math.floor(patched_input_size[1] * self.crop_percentage_axes),
            )
            ...
            row_offsets = start_x.unsqueeze(1) + torch.arange(
                h_crop, device=device
            ).unsqueeze(0)  # (B, h_crop)
            col_offsets = start_y.unsqueeze(1) + torch.arange(
                w_crop, device=device
            ).unsqueeze(0)  # (B, w_crop)
            # compute linear indices (row * W + col) and flatten per sample
            all_indices = row_offsets.unsqueeze(2) * W + col_offsets.unsqueeze(
                1
            )  # (B, h_crop, w_crop)
        ...
        mask_input, mask_output = (
            torch.sort(indices[:, :cutoff], dim=1)[0],
            torch.sort(indices[:, cutoff:], dim=1)[0],
        )
        if self.crop_percentage_axes < 1.0:
            # if we cropped the patch, we need to gather the indices from the original indices
            final_input, final_output = (
                torch.gather(all_indices, dim=1, index=mask_input),
                torch.gather(all_indices, dim=1, index=mask_output),
            )
    ...
    def _mask_Temporal(self, B, T, device="cpu"):
        ...
        indices = torch.argsort(value, dim=1)
        return torch.sort(indices[:, :cutoff], dim=1)[0], torch.sort(
            indices[:, cutoff:], dim=1
        )[0]
    ...
    def __call__(self, batch):
        ...
        mask_enc_S, mask_pred_S = self._mask_Spatial(
            B, patched_input_size, device=device
        )
        assert mask_enc_S.numel() > 0, (
            f"mask_enc_S is empty. {mask_enc_S} {patched_input_size}"
        )
        ...
        for modality in modalities:
            ...
            if len(shape) == 4:
                ...
                raise ValueError(
                    f"{modality} has an invalid shape {shape}. Must be 4 or 5"
                )
            masks_enc_C, _ = self._mask_Channel(B, C, device=device)
            masks_enc_T, _ = self._mask_Temporal(B, T, device=device)

            masks_enc[modality] = Mask(
                S=mask_enc_S,
                T=masks_enc_T,
                C=masks_enc_C,
                S_length=patched_input_size[0] * patched_input_size[1],
                T_length=T,
                C_length=C,
            )
        ...
        return [masks_enc], [
            MaskSpatial(
                S=mask_pred_S, S_length=patched_input_size[0] * patched_input_size[1]
            )
        ]
