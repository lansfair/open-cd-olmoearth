...
class Mask(object):
    ...
    def __init__(
        ...
        assert S is None or S_length is not None, (
            f"Invalid S_length {S_length}. Must be provided if S is not None"
        )
        assert T is None or T_length is not None, (
            f"Invalid T_length {T_length}. Must be provided if T is not None"
        )
        assert C is None or C_length is not None, (
            f"Invalid C_length {C_length}. Must be provided if C is not None"
        )

        assert S is None or S.numel() == 0 or S.max() < S_length, (
            f"Invalid S_length {S_length}. The mask is bigger ({S.max()})"
        )
        assert T is None or T.numel() == 0 or T.max() < T_length, (
            f"Invalid T_length {T_length}. The mask is bigger ({T.max()})"
        )
        assert C is None or C.numel() == 0 or C.max() < C_length, (
            f"Invalid C_length {C_length}. The mask is bigger ({C.max()})"
        )
    ...
    def _unsqueeze_mask(self, mask, mask_type, current_shape):
        ...
        """
        assert current_shape[0] == "B", (
            f"Invalid current shape {current_shape}. Must start with B"
        )
        ...
        assert position == -1, (
            f"mask type {mask_type} not found in current shape {current_shape}"
        )
        return mask

    def apply(
        self, x: torch.Tensor, axis="STC", current_shape="BTCSD", upsample_S_to=None
    ):
        ...
        for ax in axis:
            assert ax in ["S", "T", "C"], (
                f"Invalid axis {ax}. Must be one of ['S', 'T', 'C']"
            )
            assert ax in current_shape, (
                f"Invalid axis {ax}. Must be among the current axis : {current_shape}"
            )
            if (
                ax == "S"
                and upsample_S_to is not None
                and upsample_S_to != self.S_length
            ):
                mask, _ = self._upsample_mask(upsample_S_to)
                assert (
                    mask is None or x.shape[current_shape.index(ax)] == upsample_S_to
                ), (
                    f"Invalid upsample_S_to {upsample_S_to}. The upsampled mask length does not match the input shape {x.shape[current_shape.index(ax)]}"
                )
            else:
            ...
            if "T" in current_shape and len(x.shape) == len(current_shape) - 1:
                ...
                if ax == "T":
                    raise ValueError(
                        "Invalid axis T. The current data does not support a T dimension."
                    )
            ...
            x = torch.gather(
                x, dim=current_shape.index(ax), index=mask.expand(expand_shape)
            )
        return x
    ...
    def _upsample_mask(self, new_S_length):
        ...
        """
        assert new_S_length >= self.S_length, (
            f"Invalid new length {new_S_length}. Must be bigger than current length {self.S_length}"
        )
        if self.S is None:
            return None, 0
        ...
        row_offsets = (
            torch.arange(factor, device=self.S.device)[None, None, :, None] * new_side
        )
        # col offset (dc)
    ...
    def revert(
        ...
        """
        assert axis in current_shape, (
            f"Invalid axis {axis}. Must be among the current axis: {current_shape}"
        )
        ...
        if axis in ["T", "C"]:
            ...
            if target_S_length == -1:
                ...
                assert target_S_length >= self.S_length, (
                    f"Invalid target S length {target_S_length}. Must be bigger than current length {self.S_length}"
                )
        ...
        if mask is None or mask.numel() == 0:
            return torch.full(
                shape, fill_value=fill_value, device=x.device, dtype=x.dtype
            )

        x_reverted = torch.full(
            shape, fill_value=fill_value, device=x.device, dtype=x.dtype
        )
...
def merge_Mask(masks: List[Mask]):
    ...
    for mask in masks:
        ...
        if masks[0].C is not None:
            C.append(mask.C)
        assert mask.S_length == S_length, (
            f"Invalid S_length {mask.S_length}. Must be the same for all masks"
        )
        assert mask.T_length == T_length, (
            f"Invalid T_length {mask.T_length}. Must be the same for all masks"
        )
        assert mask.C_length == C_length, (
            f"Invalid C_length {mask.C_length}. Must be the same for all masks"
        )
    S = torch.cat(S, dim=0) if len(S) > 0 else None
    T = torch.cat(T, dim=0) if len(T) > 0 else None
    C = torch.cat(C, dim=0) if len(C) > 0 else None
    return Mask(S=S, T=T, C=C, S_length=S_length, T_length=T_length, C_length=C_length)
...
def apply_spatial_masks(x, masks: Union[Mask, List[Mask]], upsample_S_to=None):
    ...
    for m in masks:
        all_x.append(
            m.apply(x, axis="S", current_shape="BSD", upsample_S_to=upsample_S_to)
        )
