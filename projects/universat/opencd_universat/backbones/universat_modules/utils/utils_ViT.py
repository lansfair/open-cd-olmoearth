...
def _compiler_disable(function):
    """Use the PyTorch 2 compiler guard while remaining importable on 1.x."""
    compiler = getattr(torch, "compiler", None)
    disable = getattr(compiler, "disable", None)
    return disable(function) if disable is not None else function
    ...
    def forward(self, x, coords):
        ...
        if self.gating:
            ...
            q, gate_score = torch.split(
                q, [C // self.num_heads, C // self.num_heads], dim=-1
            )
    ...
    def get_qkv(self, x):
        ...
        q = torch.cat(
            [
                ...
                torch.max(x, dim=1).values.unsqueeze(1),
            ],
            dim=2,
        )
        ...
        if self.gating:
            ...
            gate_score = gate_score.reshape(
                B, self.n_queries, self.num_heads, C // self.num_heads
            )
...
class ACABlock(nn.Module):
    ...
    def forward(self, x, coords=None):
        if self.expand_dim == 2:
            res = torch.cat(
                [x.mean(dim=1).unsqueeze(1), x.max(dim=1).values.unsqueeze(1)], dim=2
            )
...
class CrossAttention(nn.Module):
    ...
    def _device_weight_init(self):
        ...
        nn.init.kaiming_normal_(self.wv.weight, nonlinearity="relu")
        self.wv.weight.data.copy_(
            torch.eye(self.wv.weight.shape[0], self.wv.weight.shape[1])
        )
    ...
    def forward(
        ...
        B, Nkv, C = kv.shape
        B, Nq, C = q.shape
        size_q = int((Nq - n_registers_q) ** 0.5)
        size_kv = int((Nkv // n_modalities - n_registers) ** 0.5)
        ...
        if self.gating:
            ...
            q = q.reshape(B, Nq, self.num_heads, C // self.num_heads).permute(
                0, 2, 1, 3
            )
        else:
            q = (
        ...
        k = self.k_norm(
            self.wk(kv)
            .reshape(B, Nkv, self.num_heads, C // self.num_heads)
            .permute(0, 2, 1, 3)
        )
        v = (
            self.wv(kv)
        ...
        if masking:
            attn_mask = matching_grids(
                size_q, size_kv, n_registers=n_registers, n_registers_q=n_registers_q
            ).to(q.device)
...
class CrossBlockLearned(nn.Module):
    ...
    def forward(
        self, kv, coords_kv, n_registers, n_modalities, grid_size, mask_out=None
    ):
...
class CrossBlock(nn.Module):
    ...
    def forward(
        self, q, coords_q, kv, coords_kv, n_registers, n_registers_q, n_modalities
    ):
        x = q + self.drop_path(
            self.attn(
                self.norm_q(q),
...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.numel() > 2**24 and not torch.is_grad_enabled:
            chunk_size = 2**24 // (
                x.numel() // x.shape[0]
            )  # keep approx 1G elements per chunk
            x_chunks = []
    ...
    def forward(self, x, coords, norm_coords=False):
        if norm_coords:
            coords = (coords - coords.amin(dim=(1, 2), keepdim=True)) / (
                coords.amax(dim=(1, 2), keepdim=True)
                - coords.amin(dim=(1, 2), keepdim=True)
            )
