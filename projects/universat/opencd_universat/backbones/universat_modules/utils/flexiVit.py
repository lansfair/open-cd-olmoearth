...
class FlexiViTLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        assert H.is_integer(), (
            f"Input spatial dimensions {H}x{W} are not square integers"
        )
        H = W = int(H)
        ...
        x = torch.nn.functional.interpolate(
            x, size=(self.HW, self.HW), mode="bilinear", align_corners=False
        )
...
class FlexiViTTemporel(nn.Module):
    ...
    def __init__(self, HW, in_channels, out_channels, nb_times=12):
        ...
        self.linear = nn.Linear(
            in_channels * HW * HW, nb_times * out_channels, bias=False
        )
        nn.init.orthogonal_(self.linear.weight)
    ...
    def forward(self, x, dates):
        ...
        H = W = math.sqrt(HWC // self.in_channels)
        assert H.is_integer(), (
            f"Input spatial dimensions {H}x{W}x{self.in_channels} are not square integers"
        )
        H = W = int(H)
        ...
        x = self.linear(x)
        x = rearrange(
            x, "b n t (tref d) -> b n t tref d", tref=self.nb_times, d=self.out_channels
        )
        ...
        if dates is not None:
            ...
            dates_remainder = repeat(
                dates - dates_inf, " b t -> b n t d", n=N, d=self.out_channels
            )

            x_inf = x.gather(
                3,
                repeat(
                    dates_inf, " b t -> b n t tref d", n=N, tref=1, d=self.out_channels
                ).long(),
            ).squeeze(3)  # B,N,T,D
            x_sup = x.gather(
                3,
                repeat(
                    dates_sup, " b t -> b n t tref d", n=N, tref=1, d=self.out_channels
                ).long(),
            ).squeeze(3)  # B,N,T,D
