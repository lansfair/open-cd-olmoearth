...
def get_coords(x, grid_size, n_modalities, register, res=1, modis=False):
    ...
    coord_x = (
        torch.linspace(0, 1, grid_size, device=x.device, dtype=torch.float32) * res
    )
    coord_y = (
        torch.linspace(0, 1, grid_size, device=x.device, dtype=torch.float32) * res
    )
    ...
    if modis:
        coords_all = torch.cat(
            [torch.zeros(b, 1, 2, device=x.device, dtype=torch.float32), coords_all],
            dim=1,
        )
    if register > 3:
        reg_coords = get_edge_coordinates(register, torch.float32, x.device).expand(
            b, -1, -1
        )
    else:
...
def get_coords_1D(x, num_patches, n_modalities, register, res=1, modis=False):
    ...
    if modis:
        coords_all = torch.cat(
            [torch.zeros(b, 1, device=x.device, dtype=x.dtype), coords_all], dim=1
        )
