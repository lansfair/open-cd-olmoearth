...
class UniversalPatchEncoder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert len(n_queries) == len(order), (
            "n_queries and order must have the same length"
        )
        assert len(n_queries) == len(expand_dim), (
            "n_queries and expand_dim must have the same length"
        )
        ...
        for modality in [
            ...
            setattr(
                self,
                "_".join(["Encoding", modality]),
                nn.Parameter(torch.randn(embed_dim), requires_grad=True),
            )
    ...
    def forward(
        ...
        for i, wv in enumerate(wavelengths):
            if isinstance(wv, str):
                ...
                coords_spectral_tensors.append(
                    getattr(self, f"Encoding_{wv}").view(1, 1, -1)
                )
            else:
                coords_spectral_tensors.append(
                    self.spectral_embed(
                        torch.tensor(wv, device=x.device, dtype=x.dtype).view(1, 1, 1)
                    )
                )
        ...
        coords_spatial = get_coords(x, int(S**0.5), 1, 4, res=1) * (
            scale * int(S**0.5) / 100
        )  # in km
        coords_sub_spatial = (
            get_coords(x, int(s**0.5), 1, 0, res=1) * (subpatch * res) / 10
        )

        if masks is not None:
            x = [
                mask.apply(
                    x.view(B_o, P, T, C, S, s, D), axis="SCT", current_shape="BSTCXXD"
                ).flatten(0, 1)
                for mask in masks
            ]
            ...
            coords_spectral = [
                mask.apply(
                    coords_spectral.view(B_o, P, C, D), axis="SC", current_shape="BSCD"
                ).flatten(0, 1)
                for mask in masks
            ]
            coords_spectral = torch.cat(coords_spectral, dim=0)  # len(masks) * B, C, D
            coords_spatial = coords_spatial[
                : x.shape[0]
            ]  # it is the same for all masks
            coords_sub_spatial = coords_sub_spatial[
                : x.shape[0]
            ]  # it is the same for all masks
        ...
        coords_temporal = torch.cat(
            [
                torch.zeros(
                    x.shape[0], self.registers_temporal.shape[1], device=x.device
                ),
        ...
        for i in range(len(self.order)):
            ...
            if self.order[i] == "C":
                ...
                if coords_spectral is not None:
                    D_expand = D // D_orig
                    cs = (
                        coords_spectral.repeat(1, 1, D_expand)
                        if D_expand > 1
                        else coords_spectral
                    )
                ...
                if T > 1:
                    x = torch.cat(
                        [self.registers_temporal.repeat(B * C * S, 1, 1), x], dim=1
                    )
                else:
                    coords_temporal = coords_temporal[
                        :, self.registers_temporal.shape[1] :
                    ]
                ...
                x = x.flatten(0, 2)
                x = torch.cat(
                    [self.registers_spatial.repeat(B * T * C, 1, 1), x], dim=1
                )
                ...
                x = x.flatten(0, 3)
                x = blk(
                    x, coords=coords_sub_spatial.repeat_interleave(S * T * C, dim=0)
                )
                D = D * self.expand_dim[i]
...
def repeat_interleave_batch(x, B, repeat):
    ...
    x = (
        x.unsqueeze(1)
        .expand(N, repeat, B, *x.shape[2:])
        .reshape(N * repeat * B, *x.shape[2:])
    )
    return x
